...
type TText = "create" | "update";
type TTool = CreateProduct | UpdateProduct;
type TUseFun = {
  (type:TText, product:TTool): string
}
export const submitProduct:TUseFun = (type, product) => {
  if (type === "create") {
    const newProduct = product as CreateProduct;
    return `新增商品成功：${newProduct.title}`;
  } else {
